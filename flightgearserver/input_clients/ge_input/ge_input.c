...
void process_line(char buffer[], char server_host[], int server_port){
	...
	if (count == 13){
		...
		Flight_info flight_info = new_flight_info(time_stamp, position, orientation);
...
void input_flight_info_from_file(char file_name[], char server_host[], int server_port) {
	...
	while(valid_buffer_start < valid_buffer_end){
		...
		process_line(buffer, server_host, server_port);
	}
}
