  ...
  //for (i = (int) (lenghtXmnt / 2 + start / stepXmnt), j = start; j <= end; j += stepXmnt, i++)
	...
	for (i = (int) (lenghtXmnt / 2 + start / stepXmnt), j = start; j <= end; j += stepXmnt, i++)
	 ...
	 if(id >= 0 && id <= 65535 && idx >= 0 && idx <= 255)// && idy >= -65536 && idy <= 65535)
	   {
	     glColor3fv(pdfterraincolor[id]);
	     glVertex3f(j, pdfterrain[id], k + stepYmnt);
	   }
       }
...
static void loadpdf(void)
{
  ...
  bayesrun.run_grid(gettime(),pdfdata, camera_pos);  
  double scale_factor = get_scale_factor(pdfdata);// * boostPDF;

  for (i = 0; i < (256 * 256); i++)
   {
     bufferter[i] = (GLubyte)(pdfdata(i) * scale_factor);
     pdfterrain[i] = (bufferter[i]);
...
int main(int ac, char **av)
{
  ...
  bayesrun.grid_init();  //initialise bayesian algorithm
  bayesrun.run_grid(0.0,blank, camera_pos);
